...
describe('DataInputPage Component', () => {
  ...
  describe('TC-DATA-001: ページ表示確認', () => {
    it('should render page title and description', () => {
      ...
      expect(screen.getByText(/記入済みアンケートを撮影してOCR処理を行い/)).toBeDefined();
    });
    ...
    it('should display step indicators', () => {
      ...
      expect(screen.getByText('ステップ 2: アンケート撮影・OCR実行')).toBeDefined();
    });
  ...
  describe('TC-DATA-002: テンプレート選択', () => {
    ...
    it('should display message when no templates exist', () => {
      ...
      expect(screen.getByText('テンプレートが登録されていません')).toBeDefined();
    });
  });
  ...
  describe('TC-DATA-007: クリップボードコピー', () => {
    ...
    it('should handle clipboard copy failure', async () => {
      ...
      // エラーメッセージの表示確認
      await waitFor(() => {
        expect(screen.getByText('クリップボードへのコピーに失敗しました')).toBeDefined();
      });
  ...
  describe('Edge Cases', () => {
    it('should handle undefined template gracefully', () => {
      ...
      const template1Button = screen.getByText('テンプレート1').closest('button');
